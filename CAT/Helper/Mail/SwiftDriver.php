...
namespace CAT\Helper\Mail;

use \CAT\Base as Base;

if (!class_exists('\CAT\Helper\Mail\SwiftDriver'))
{
    class SwiftDriver extends Base
    {
        ...
        {
            require_once CAT_ENGINE_PATH.'/CAT/vendor/swiftmailer/swiftmailer/lib/swift_required.php';
        ...
        public function sendMail($fromaddress, $toaddress, $subject, $message, $fromname='', $html='')
        {
            ...
            if (!self::$transport) {
                ...
                if (isset(self::$settings['routine'])
                    ...
                    self::$transport = new \Swift_SmtpTransport(self::$settings['smtp_host'], $port, $tp);
                    $use_smtp = true;
                } else {
                    self::$transport = new \Swift_NullTransport();
                }
                if ($use_smtp
            ...
            try {
                ...
                self::addError('Transport exception: '.$e->getMessage().' (Port: '.$port.', Transport: '.$tp.')');
                return false;
            } catch (Exception $e) {
