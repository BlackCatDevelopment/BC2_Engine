...
{foreach $dashboard.columns col name='outer'}
    <ul style="width:{$col.width}%;" class="widgets dashboard_sortable" id="dashboard_col_{$.foreach.outer.index}" data-module="{$module}">
    {foreach $col.widgets key widget name='inner'}
        <li class="widget_wrapper clearfix" id="dashboard_col_{$.foreach.outer.index}_item_{$.foreach.inner.index}" data-widget="{$widget.widget_path}">
			...
			<div class="widget_content">{$widget.content}</div>
		</li>
    {/foreach}
    </ul>
{/foreach}
